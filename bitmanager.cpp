...
bitmanager::bitmanager(int length, char *data)
{
    if(length<=0) {
    //强行限制最小长度为8（当输入值小时）
    length=1;
  ...
  realData=data;
  bmLength=length;
  //effectiveLength=(eLength<=length*8)?eLength:length*8; //有效长度不能大于最大
}
...
bool bitmanager::atPos(int n)  //取出第n位的值
{
...
bool bitmanager::setBitmap(int pos, bool value)  //设置
{
